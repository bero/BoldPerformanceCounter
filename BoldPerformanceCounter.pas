 ...
 * https://github.com/Embarcadero/BoldForDelphi/blob/develop/Source/BoldPerformanceCounter.pas   *
 * with some minor changes to remove dependency                                                  *
 *                                                                                               *
 *************************************************************************************************}
...
begin
  result := DateUtils.SecondSpan(fOwnData.AccumulatedTime + fChildrenData.AccumulatedTime, 0);
end;
...
begin
  if Assigned(fOwner) then
  begin
    var ownerTotalTime := fOwner.fOwnData.fAccumulatedTime + fOwner.fChildrenData.fAccumulatedTime;
    if ownerTotalTime > 0 then
      result := 100 * (fOwnData.fAccumulatedTime + fChildrenData.fAccumulatedTime) / ownerTotalTime
    else
      result := 0;
  end
  else
...
begin
  ...
  begin
    ...
    if ChildCount > 0 then
      result := format('%-50s %8.6f sec (own: %8.6f sec) calls: %5d ', [lIndent+fName, Seconds, SecondsWithoutChildren, Calls])
    else
      result := format('%-50s %8.6f sec calls: %5d ', [lIndent+fName, Seconds, Calls]);
  ...
  for i := 0 to ChildCount - 1 do
    result := result + Children[i].AsDetailedString + sLineBreak;
end;
